...
const countChangePluginKey = new PluginKey('countChangePlugin');
  ...
  return new Plugin({
    ...
    state: {
      ...
      apply(tr, value, oldState, newState) {
        ...
        // 上面两种情况不增加修改次数
        if((onlyContentChanged && !tr.docChanged) || (editorView && editorView.composing)) {
          return value
        }
...
class DocChangedView implements PluginView {
  ...
  private updateTimesView(state: EditorState) {
    this.recordChangedTimesDom.innerText = `文档被修改了${countChangePluginKey.getState(state).times} 次`
  }
