...
export function insertHeading(editorView: EditorView, content: string, level = 1) {
  ...
  const heading = schema.node('heading', { level }, schema.text(content))
...
export function insertImage(editorView: EditorView, src, alt = '', title = '') {
  const { state, dispatch } = editorView
  const schema = state.schema as Schema;

  const image = schema.nodes.image.create({ src, alt, title });
